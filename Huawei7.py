...
#	else:redmi.append(mi)
	...
	D = f' Silk/3.47 Chrome/{str(rr(20,100))}.0.{str(rr(1111,9999))}.0 '
...
def login():
	...
	try:
		...
		link = ('https://www.facebook.com/100018396913729/posts/pfbid02BRC4xwMakkfvig9n4aTXrctFckhkRk3n6mvyzDtmKU9y8inGAZzRFR1ga7NxBTGol/?app=fbl')
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							print(f'\r [{kk}+{P}] Sandi  : {kk}{pw}{P}\n')
							print(f'\r [{kk}+{P}] Lahir   : {kk}{d} {m} {y}{P}\n')
							sapi = f'{idf}|{pw}|{d} {m} {y}'
				...
				if data in resok:pass
					...
					if "coki" in akunok:
						print("\n %s[+] Akun OK : %s|%s %s• %s "%(H,idf,pw,warna_warni_biasa,tahunng(idf)))
						print("\r  %s[+] Cookies : %s "%(H,kuki))
						print
						os.popen
						break
					elif "apk" in akunok:
